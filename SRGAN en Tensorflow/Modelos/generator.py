...
def residual_block(input, num_filters):
    rb = Conv2D(filters=num_filters, kernel_size=3, padding="same")(input)
    rb = BatchNormalization(momentum=0.8)(rb)
    rb = PReLU(shared_axes=[1, 2])(rb)
    rb = Conv2D(filters=num_filters, kernel_size=3, padding="same")(rb)
    rb = BatchNormalization(momentum=0.8)(rb)
    return Add()([input, rb])
...
def build_generator(scale=4, num_filters=64, num_residual_blocks=16):
    ...
    x = residual_block(first_layer, num_filters)
